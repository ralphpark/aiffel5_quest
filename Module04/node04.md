...
- 자율주행자동차에서 state, action, reward를 찾아보자
- 안전하게 끼어들면서 각 색깔의 자동차가 각자의 목적지에 가는것이 목표
- state: 자동차의 위치(1,2,3차선 혹은 1,5차선 2.5차선), 차량의 색상, 내차 혹은 상대차 속도, 주변 차량의 위치, 차선의 위치, 신호등의 색깔
- 너무 많은 state를 주면 학습이 안되거나 찾기가 어려워진다
- action: 가속, 감속, 앞뒤좌우로 움직이기
- reward: 목적지에 도착하면 +100, 신호등을 무시하면 -100, 사고를 당하면 -10000
- 사고를 당하지 않게 하기 위해서 reward를 -10000으로 설정하면 매우 천천히 간다.
- 빠르게 가면 +100, 느리게 가면 -100 같이 설계하는 것에 따라서 학습이 달라진다.
...
- 양궁로봇에서 state, action, reward를 찾아보자
- 양궁로봇은 화살을 쏘는 것이 목표
- state: agent로 부터 과녁의 거리, 바람의 세기와 방향, 화살의 위치, 화살의 속도
- action: 화살을 쏘는 힘, 화살을 쏘는 각도, 회전각
- reward: 화살이 정확하게 맞으면 +100, 화살이 맞지 않으면 -100
...
- 강화학습은 어디에 활용할까?
- 강화학습은 사람의 목슴과 관련된 경우에는 활용하기 좋지 않다
- 광고 레이아웃 결정
![](./img/rein_03.png)
- 광고 배너와 문구가 어디에 배치되어야 클릭율이 올라갈지를 강화학습을 통해 알아낼 수 있다.
- 강화학습을 적용한 응용분야는?
- 사람의 안정성에 위해를 끼치지 않는 케이스가 적절하다
- Thumbnail Selection : 유튜브의 썸네일을 강화학습을 통해 선택

Q. 강화학습이 활용되면 좋지 않은 경우에는 어떤 것이 있을까요?
- 수술이나 자율주행자동차와 같이 사람이 위험한 상황에 빠지게 되는 경우 좋지 않습니다. 무수히 많은 state를 학습시키지 않고는 사람의 생명이 위태로울 수 있습니다.

Q. 강화학습을 적용할 수 있는 응용분야에는 무엇이 있을까요?
- 광고 레이아웃 결정, Thumbnail Selection, 자율주행자동차, 양궁로봇 등
- 
- 강화학습은 보상을 최대화하는 에이전트의 행동을 학습하는 방식으로 머신러닝 문제를 해결하는데 적용됩니다. 다양한 응용 분야에서 활용되고 있습니다. 아래는 강화학습이 적용된 대표적인 응용분야들입니다.

1. 게임: 강화학습은 AlphaGo를 비롯한 대표적인 게임 분야에서 활용됩니다. 게임은 보상 최대화 문제를 잘 모델링할 수 있는 분야로, 이를 통해 인공지능이 게임에서도 높은 수준의 성능을 보일 수 있게 되었습니다.

2. 자율주행차: 자율주행차는 복잡한 도로 상황에서 신속하고 정확한 판단이 필요한 기술입니다. 강화학습은 이를 위한 학습 방법으로 사용되며, 학습을 통해 도로 상황에 대한 판단력 및 주행 안전성이 향상될 수 있습니다.

3. 로봇제어: 강화학습은 로봇 제어 분야에서도 활용됩니다. 로봇을 제어하는 데 사용되는 정책 학습에서 많이 활용됩니다. 로봇 제어에서는 보상 함수를 적절하게 설정하면 최적의 정책을 학습할 수 있습니다.

4. 자원 관리: 자원 관리는 일정 기간 동안 최적의 자원 사용을 결정하는 문제입니다. 예를 들어, 전력 분야에서 강화학습은 전력 사용량을 최적화하여 비용을 절감합니다.

5. 판매 자동화: 판매자는 기계 학습을 통해 고객의 구매 또는 요구 사항에 대한 타당한 계획을 미리 세울 수 있습니다. 이를 통해 효과적인 의사결정, 자원 사용 및 재고 관리를 지원할 수 있습니다.

이 외에도, 강화 학습은 자연어 처리, 의료, 환경 등 다양한 분야에서 활용되고 있습니다.